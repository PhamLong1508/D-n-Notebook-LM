...
const menu = [
  ...
  { key: "/notebooks", icon: <BookOpenIcon className="w-6 h-6" />, label: "Notebooks" },
];
...
export default function MainLayout() {
  ...
  const { snippets, removeSnippet, clearContext, noteCreator } = useContextMenu();

  const handleCreateNote = () => {
    if (noteCreator) {
        const combinedContent = snippets.map(s => s.content).join("\n\n---\n\n");
        ...
        <div className="p-4 border-t border-gray-200">
          {user && (
            <div className="flex items-center justify-between">
                ...
                <button onClick={handleLogout} className="p-2 rounded-md hover:bg-gray-100 text-gray-500 hover:text-gray-700">
        ...
        <aside
          ...
          {snippets.length === 0 ? (
            ...
            <div className="flex-1 overflow-y-auto pr-2 space-y-4">
              {snippets.map((snippet) => (
                <div key={snippet.id} className="bg-gray-100 p-3 rounded-md relative group">
          ...
          {snippets.length > 0 && (
            <div className="mt-4 pt-4 border-t">
                <Button type="primary" block icon={<EditOutlined />} onClick={handleCreateNote}>
                    Tạo ghi chú mới
                </Button>
            </div>
          )}
